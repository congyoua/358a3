...
class Node:
    ...
    def update(self, pkt: Packet):
        ...
        print(self.nodeid, "got", vector, "from", src, "was", self.dist_table)
        if self.dist_table[src] != vector:
            self.dist_table[src] = vector
            for target in range(NUM_NODES):
                self.dist_table[self.nodeid][target] = self.get_link_cost(target)
                self.predecessors[target] = target
                for i in range(NUM_NODES):
                    if self.get_link_cost(i) + self.dist_table[i][target] < \
                            self.dist_table[self.nodeid][target]:
                        self.dist_table[self.nodeid][target] = \
                            self.get_link_cost(i) + self.dist_table[i][target]
        ...
        #print(self.nodeid, "got", vector, "from", src, "now", self.dist_table)
        #print(self.nodeid, "sent", self.dist_table[self.nodeid])
        #print(self.nodeid, "dvtable", self.simulator.cost[self.nodeid])
    ...
    def link_cost_change_handler(self, which_link: int, new_cost: int):
        ...
        for i in range(NUM_NODES):
            if i != self.nodeid and self.get_link_cost(i) != inf:
                self.simulator.to_link_layer(Packet(self.nodeid, i, self.dist_table[self.nodeid]))
        #print(self.nodeid, "change", "now", self.dist_table)
